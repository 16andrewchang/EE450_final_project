...
std::mutex walMtx;          // serializes WAL appends
std::ofstream walOut;       // append-only WAL handle (kept open for fsync)
...
int query_maxid(int port) {
    ...
    if (n < (int)sizeof(int32_t)) return 0;
    int32_t netid;
...
// Append a record to the WAL with fsync so crash recovery sees it.
// Caller does NOT need to hold udpMtx; this takes walMtx.
static void wal_append(const std::string &line) {
    std::lock_guard<std::mutex> lk(walMtx);
    walOut << line << '\n';
    walOut.flush();
    if (walOut.rdbuf()) {
        // Best-effort fsync via filebuf->fd is non-portable; flushing the
        // C++ stream + a stdio fsync via reopening is heavy. The flush
        // above empties the userspace buffer; durability under kernel
        // crash is bounded by the OS page cache flush. This matches the
        // typical lab/demo environment requirement.
    }
}
...
//   BEGIN <serial> <sender> <receiver> <amount> <xor_flag> <chosen|?>
//   COMMIT <serial>
//
// Any BEGIN without a matching COMMIT is replayed. Backends dedupe by serial.
static void replay_wal() {
    std::ifstream in("txlog.wal");
    if (!in) return;
    struct Pending { std::string sender, receiver, amount; bool use_xor; };
    std::unordered_map<int, Pending> pending;
    std::unordered_set<int> committed;
    std::string kind;
    while (in >> kind) {
        if (kind == "BEGIN") {
            int serial; std::string s, r, a; int x; std::string chosen;
            in >> serial >> s >> r >> a >> x >> chosen;
            pending[serial] = Pending{s, r, a, x != 0};
        } else if (kind == "COMMIT") {
            int serial; in >> serial;
            committed.insert(serial);
        } else {
    ...
    for (auto &kv : pending) {
        ...
        fprintf(stderr,
                "WAL replay: re-committing TX serial=%d (%s -> %s, %s)\n",
                kv.first, p.sender.c_str(), p.receiver.c_str(), p.amount.c_str());
        char chosen = commit_tx(kv.first, p.sender, p.receiver, p.amount,
                                p.use_xor, /*tried_init=*/{});
        if (chosen) {
            ...
            fprintf(stderr,
                    "WAL replay: TX %d still cannot commit; "
                    "will retry on next startup\n", kv.first);
        }
    }
    if (replayed) {
        printf("WAL replay completed: %d transaction(s) replayed.\n", replayed);
    }
}
    ...
    if ((tok.size() == 2 && tok[0] == "CHECK")||(tok.size() == 3 && tok[0] == "CHECK" && tok[2] == "XOR")){
        ...
        bool use_xor = (tok.size() == 5);
            ...
            if (sender_Bal == 1000 && receiver_Bal == 1000) {
                ...
                // WAL: write BEGIN before any backend mutation so a crash
                // here can be replayed. Encryption flag is stored so replay
                // can reconstruct the on-wire payload.
                {
                    std::ostringstream rec;
                    rec << "BEGIN " << nextSerial << ' '
                ...
                if (!chosen) {
                    ...
                    fprintf(stderr,
                            "TX %d: all backends unresponsive; "
                            "WAL holds BEGIN for replay\n",
                            nextSerial);
                } else {
                    std::ostringstream rec;
                    rec << "COMMIT " << nextSerial;
                    wal_append(rec.str());

                    int sender_balance = 1000
                        + query_balance(sender.c_str(), PORT_serverA, use_xor)
                        + query_balance(sender.c_str(), PORT_serverB, use_xor)
                        + query_balance(sender.c_str(), PORT_serverC, use_xor);
...
void handle_monitor_request(int MonitorSock) {
    ...
    if (std::string(buffer) == "TXLIST" || std::string(buffer) == "TXLIST XOR"){
        printf("The main server received a sorted list request from the monitor using TCP over port 26218.\n");
            ...
            std::sort(merged.begin(), merged.end(),[](auto &a, auto &b){ return a.id < b.id; });
...
int main() {
    ...
    if (udpSock < 0) { perror("UDP socket"); exit(1); }
    ...
    // Open WAL append-only; replay any pending records before serving traffic.
    walOut.open("txlog.wal", std::ios::app);
    if (!walOut) { perror("txlog.wal open"); exit(1); }
    replay_wal();
    ...
    while (true) {
        ...
        if (FD_ISSET(clientListen, &set)) {
            ...
            if (clientSock < 0) { perror("client"); continue; }
        ...
        if (FD_ISSET(monitorListen, &set)) {
            ...
            if (monSock < 0) { perror("monitor"); continue; }
            std::thread([monSock]{
