...
int main() {
    std::ifstream fin("block2.txt");
    if (!fin) { perror("block2.txt open"); exit(1); }
    std::vector<Tx> txs;
    while (!fin.eof()) {
        Tx t;
    ...
    while (1) {
        ...
        if (tok.size()==2 && tok[0]=="TXLIST" && tok[1]=="SHIFT") {
            ...
            sendto(sockfd, out.data(), out.size(), 0,(sockaddr*)&mainServAddr,len);
            ...
            if(tok[0]=="XOR"){
             ...
             fprintf(stderr, "DEBUG: decrypted user = \"%s\"\n", dec_user.c_str());
            ...
            // Idempotent: a coordinator retry must not double-append.
            if (seen_ids.count(serial)) {
                const char* ack = "LOG_OK";
                sendto(sockfd, ack, strlen(ack), 0,
                       (sockaddr*)&mainServAddr, len);
            } else {
                std::ofstream fout("block2.txt", std::ios::app);
                if (!fout) {
                    perror("block2.txt append");
                }
                else {
                    fout<< '\n'
                        << serial << ' '
            ...
            sendto(sockfd, err, strlen(err), 0,
                   (sockaddr*)&mainServAddr, len);
            fprintf(stderr, "ServerB: malformed request: %s\n", buffer);
        }
